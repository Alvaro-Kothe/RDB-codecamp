...
def calculate_demographic_data(print_data=True):
    ...
    rich_percentage = ((df.loc[df["hours-per-week"] == min_work_hours, "salary"] == ">50K").mean() * 100).round(1)

    # What country has the highest percentage of people that earn >50K?
    highest_earning_country = df.groupby("native-country")["salary"].apply(lambda x: (x == ">50K").mean()).idxmax()
    highest_earning_country_percentage = ((df.loc[df["native-country"] == highest_earning_country, "salary"] == ">50K").mean() * 100).round(1)

    # Identify the most popular occupation for those who earn >50K in India.
    top_IN_occupation = df.loc[(df["native-country"] == "India") & (df["salary"] == ">50K"), "occupation"].mode().iat[0]
